...
gFile="audio_cloverHDMI-110.command_v0.17 - BETA"
...
echo "for any reason without permission. The audio_cloverHDMI script is" 
...
# gigfxport5audio=$(cat /tmp/IGPU.txt | grep -c "audio-codec-info")
# gigfxport5connector=$(cat /tmp/IGPU.txt | grep connector-type | sed -e 's/.*<//' -e 's/>//')
...
if [ $gssdtinstall = y ]; then # install ssdt
gconnectoredit=y
    if [ $gDebug = 2 ]; then
...
# download hdef ssdt, HD3000, HD4000, HD530, HD540
...
# install ssdt to EFI/CLOVER/ACPI/patched (cloverHDMI)
case $gDebug in
...
HD6200|HD540* ) # connector detection, 530 removed special case
...
if [ $gigfxname = "HD530" ]; then # 1/gigfxname=HD530
iaudio=1

else  # 1/gigfxname=HD530
...
if [ $gigfxname = "HD530" ]; then # 2/gigfxname=HD530
index=1
patch[1]=$(($gigfxindex))
# patch=( 0 13 )

else  # 2/gigfxname=HD530
# patch framebuffer
if [ $gigfxhdmihdau = y ]; then
    index=0
    port=5
...
rm -R /tmp/config-audio_cloverHDMI.plist.zip
rm -R /tmp/ktp.plist
# rm -R /tmp/config.plist
...
if [ $gdgfx != 0 ]; then  # AMD/Nvidia else nuc
